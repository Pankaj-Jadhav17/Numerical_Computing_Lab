...
// Read least squares input file
// Format: n, then n lines of "x f"
bool readLeastSquaresInput(const string& path,
                           vector<double>& xs, vector<double>& fs) {
    ifstream fin(path);
    if (!fin) return false;
    string line;
    int n = 0;
...
void runLeastSquares() {
    ...
    // Read data from file
    vector<double> xs, fs;
    if (!readLeastSquaresInput("input/input_leastsquares.txt", xs, fs)) {
    ...
    // Print data table with sums — matches notes table layout
    write("Data Points Table:\n");
    write("  -----------------------------------------------------------\n");
    ...
    for (int i = 0; i < N; i++) {
        double xi2 = xs[i] * xs[i];
        double fxi = fs[i] * xs[i];
        sumX  += xs[i];
        sumX2 += xi2;
        sumF  += fs[i];
        sumFX += fxi;
    ...
    // Construct object and solve
    try {
        LeastSquares ls(xs, fs);
        ...
        // Print residuals table — matches notes column (yi - fi)²
        write("\nResiduals Table:\n");
        write("  -------------------------------------------------------\n");
        ...
        double S = 0.0;
        for (int i = 0; i < N; i++) {
            double yi_fit = ls.evaluate(xs[i], a, b);
            double resid  = fs[i] - yi_fit;
            double resid2 = resid * resid;
            S += resid2;
            snprintf(buf, sizeof(buf),
                     "  %3d  %7.4f  %7.4f  %10.4f  %9.4f  %9.4f\n",
                     i, xs[i], fs[i], yi_fit, resid, resid2);
            write(string(buf));
        }
        write("  -------------------------------------------------------\n");
        snprintf(buf, sizeof(buf),
...
//         snprintf(buf2, sizeof(buf2), "  Sum(λ)     = %.6f  (= trace of A)\n", traceSum);
//         write(string(buf2));
//     };
...
//     if (!fout.is_open()) {
//         cerr << "[ERROR] Cannot open output/output.txt for writing.\n";
//         cerr << "        Make sure the output/ folder is accessible.\n";
//         return 1;
//     }
